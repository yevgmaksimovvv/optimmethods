...
def _build_theory_conclusion(
    ...
    if not stationary_points:
        return "Искомый экстремум в рамках текущей модели аналитически не выделен."
...
def _build_goal_alignment(goal: str, stationary_point_kind: str) -> str:
    ...
    if goal == "min" and stationary_point_kind == "локальный минимум":
        return f"Постановка на поиск {goal_label} согласуется с аналитическим выводом."
    if stationary_point_kind == "локальный максимум":
        return f"Постановка на поиск {goal_label} не согласуется с аналитическим выводом: обнаружен локальный максимум."
    if stationary_point_kind == "локальный минимум":
        return f"Постановка на поиск {goal_label} не согласуется с аналитическим выводом: обнаружен локальный минимум."
    if stationary_point_kind == "седловая точка":
        return f"Постановка на поиск {goal_label} не согласуется с аналитическим выводом: обнаружена седловая точка."
