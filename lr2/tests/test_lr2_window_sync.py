...
from lr2.domain.models import BATCH_STATUS_DOMAIN_REFUSAL, BatchItemResult, BatchResult, SolverResult
...
def test_lr2_window_syncs_summary_iterations_and_plot(qapp_offscreen, start_point: tuple[float, float]) -> None:
    app = qapp_offscreen
...
def test_lr2_window_shows_empty_state_when_run_has_no_detail_payload(qapp_offscreen) -> None:
    app = qapp_offscreen
    window = RosenbrockWindow()
    ...
    assert window.summary_table.rowCount() == 1
    assert window.batch_summary_label.text().startswith("Комбинаций: 1")
...
def test_lr2_window_shows_domain_failure_row(qapp_offscreen) -> None:
    app = qapp_offscreen
    window = RosenbrockWindow()
    batch_result = BatchResult(
        polynomial=build_polynomial("F1", VARIANT_PRESETS["variant_f1"]),
        ...
        items=(
            BatchItemResult(
                ...
                message="Ожидаемый доменный отказ",
    ...
    assert window.summary_table.rowCount() == 1
    assert window.summary_table.item(0, 6).text() == "Доменный отказ"
    assert window.summary_table.item(0, 7).text() == "Ожидаемый доменный отказ"
    assert window.steps_table.rowCount() == 0
    assert "Ожидаемый доменный отказ" in window.steps_state_label.text()
    assert window.canvas.figure.axes
    assert any(
        "Ожидаемый доменный отказ" in text.get_text()
        for text in window.canvas.figure.axes[0].texts
    )
